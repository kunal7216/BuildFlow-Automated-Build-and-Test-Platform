...
public class DockerExecutionService {
  ...
  public DockerExecutionService(StringRedisTemplate redis, MeterRegistry meterRegistry){ this.redis = redis; this.meterRegistry = meterRegistry; }
  ...
  public Path executeBuild(BuildRun run, String yamlContent, LogCallback cb) throws Exception {
    ...
    try {n      if(yamlContent!=null && !yamlContent.isBlank()){
      ...
      for(String cmd : commands){        ProcessBuilder pb = new ProcessBuilder("sh","-c", cmd);        pb.directory(workspace.toFile());        pb.redirectErrorStream(true);        Process proc = pb.start();        BufferedReader r=new BufferedReader(new InputStreamReader(proc.getInputStream()));        String line;        while(true){          if((line=r.readLine())!=null){            cb.onLine(line);          } else {            // no more immediate lines; check if process exited or continue reading after short sleep            if(!proc.isAlive()) break;            // check cancellation            try { if(Boolean.TRUE.equals(redis.hasKey("codeflow:cancel:"+run.getRunId()))) { proc.destroyForcibly(); try{ meterRegistry.counter("codeflow_build_cancelled_total").increment(); } catch(Exception ignore){} throw new RuntimeException("Build cancelled"); } } catch(Exception ignore){}            // check timeout            long elapsed = System.currentTimeMillis()-startMillis;            if(elapsed > timeoutSeconds*1000){ proc.destroyForcibly(); try{ meterRegistry.counter("codeflow_build_timeouts_total").increment(); } catch(Exception ignore){} throw new RuntimeException("Build timed out"); }            Thread.sleep(200);            continue;          }        }        long remaining = timeoutSeconds*1000 - (System.currentTimeMillis()-startMillis);        if(remaining <= 0){ proc.destroyForcibly(); try{ meterRegistry.counter("codeflow_build_timeouts_total").increment(); } catch(Exception ignore){} throw new RuntimeException("Stage timed out"); }        boolean exited = proc.waitFor(remaining, TimeUnit.MILLISECONDS);        if(!exited){ proc.destroyForcibly(); try{ meterRegistry.counter("codeflow_build_timeouts_total").increment(); } catch(Exception ignore){} throw new RuntimeException("Stage timed out"); }        int exit = proc.exitValue();        if(exit!=0) throw new RuntimeException("Stage failed exit="+exit);      }      // return workspace for artifact collection; do not delete here      return workspace;    } catch(Exception ex){      try { Files.walk(workspace).sorted(Comparator.reverseOrder()).map(Path::toFile).forEach(File::delete); } catch(Exception ignore){}      throw ex;    }  }}
